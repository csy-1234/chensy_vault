...
### 十、wireguard
**WireGuard 本身是点对点（peer-to-peer）架构，没有严格区分客户端/服务端**。你本地的配置（比如 `wg0`）只要连到对方，就是“客户端角色”
配置模板
```
服务端（最简）
[Interface]
...
中心服务器和下游 Peer 是否用同一条命令安装？✅ 是的，完全相同

2.生成密钥
cd /etc/wireguard
wg genkey | tee privatekey | wg pubkey > publickey  #当前目录生成

3.下游节点生成配置文件
[Interface]
Address = 10.2.15.111/24  #
PrivateKey = mOOWv6jGzdEUV2f7Z1jw/BIbkrrElxzY+rk05hmpznA=
MTU = 1420
#yvnet
[Peer]
Endpoint = 111.228.0.72:7002
PublicKey = uH5ElBBkwTRxltdjJxarxUOhHez4YqWhvpR2m24wtAg=
AllowedIPs = 10.2.15.0/24
PersistentKeepalive = 21

3.启动
sudo ls /etc/wireguard   #查看接口名（conf文件去掉文件名）
sudo wg-quick up yvne    #启动接口
sudo wg show             #查看结果1
ip a                     #查看结果2
```